...
npm run easy -- dashboard --host 0.0.0.0       # 局域网/远程查看（自动生成访问令牌）
...
**传输安全边界**：令牌经明文 HTTP 传输时可被同网监听者截获。直连 `--host` 适用于**可信内网**（如本团队办公室局域网）；跨不可信网络远程查看时，请将面板置于 HTTPS 反向代理（如 Nginx/Caddy）或可信隧道（如 SSH 端口转发 `ssh -L 8800:127.0.0.1:<port>` 后在本机回环打开）之后——此时面板无需绑定非回环地址。浏览器打开即可查看：
