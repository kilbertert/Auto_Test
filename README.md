...
默认仅绑定本机回环地址（`127.0.0.1`，端口自动分配）。使用 `--host` 绑定非回环地址时，面板会**强制要求访问令牌**（未指定则自动生成），打印的 URL 中已附带令牌（如 `http://<host>:<port>/?token=…`）；没有令牌的请求一律 401。页面内的实时事件（SSE）与证据链接会自动携带令牌，请仅把完整 URL 分享给可信查看者。浏览器打开即可查看：
