...
**公网隧道模式（frp，本服务器推荐方案）**：面板保持回环绑定 + 固定端口（`--port 11880`，已登记端口注册表）+ 显式 `--token`；由 frpc 隧道（`auto-test-dashboard-frp.service`）转发到公网 HTTPS 域名。访问需要**两重凭据**：隧道层 Basic Auth（配置于 frpc，`~/.config/frp/auto-test-dashboard.toml`）+ 面板 token；HTTPS 在公网侧终结，token 不经明文传输。**显式指定的 `--token` 在回环绑定下同样强制校验**——因为回环端口可经隧道被外部触达，"回环绑定"不等于"仅本机流量"。浏览器打开即可查看：
