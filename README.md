...
```bash
npm run easy -- dashboard                                   # 仅本机回环（默认）
npm run easy -- dashboard --host 0.0.0.0                    # 局域网/内网直连（自动生成访问令牌）
npm run easy -- dashboard --host 0.0.0.0 --token <自定令牌>
npm run easy -- dashboard --port 11880 --token <面板令牌>    # 公网隧道模式（见下）
```

默认仅绑定本机回环地址（`127.0.0.1`，端口自动分配）。使用 `--host` 绑定非回环地址时，面板会**强制要求访问令牌**（未指定则自动生成），打印的 URL 中已附带令牌（如 `http://<host>:<port>/?token=…`）；没有令牌的请求一律 401。页面内的实时事件（SSE）与证据链接会自动携带令牌，请仅把完整 URL 分享给可信查看者。

**传输安全边界**：令牌经明文 HTTP 传输时可被同网监听者截获。直连 `--host` 适用于**可信内网**（如本团队办公室局域网）；跨不可信网络远程查看时，请将面板置于 HTTPS 反向代理（如 Nginx/Caddy）或可信隧道（如 SSH 端口转发 `ssh -L 8800:127.0.0.1:<port>` 后在本机回环打开）之后——此时面板无需绑定非回环地址。

**公网隧道模式（frp，本服务器推荐方案）**：面板保持回环绑定 + 固定端口（`--port 11880`，已登记端口注册表）+ 显式 `--token`；由 frpc 隧道（`auto-test-dashboard-frp.service`）转发到公网 `https://auto-test.ranlei.work`。访问需要**两重凭据**：隧道层 Basic Auth（配置于 frpc，`~/.config/frp/auto-test-dashboard.toml`）+ 面板 token；HTTPS 在公网侧终结，token 不经明文传输。**显式指定的 `--token` 在回环绑定下同样强制校验**——因为回环端口可经隧道被外部触达，"回环绑定"不等于"仅本机流量"。浏览器打开即可查看：
