...
| DeepSeek Harness | `dsh`（实验性） | 官方 SDK JSON-RPC runtime | 文件 artifact + Core 校验 | DSH 持久 Session | `DshModelProviderAdapter`：Anthropic Messages Profile -> 隔离 `settings.yaml` |

DSH 路线 B 接入官方 SDK JSON-RPC runtime，使用 `initialize`、`session/prompt`、`shutdown` 和 Session 通知，不维护第二套会话协议。Auto-Test 会为每个 Run 写入内置 Cordis 组合配置，并注入 Playwright/Control MCP；`AUTO_TEST_DSH_CORDIS_TEMPLATE` 仅用于高级覆盖。未安装可解析这些官方插件的 DSH runtime 时 Host 必须 fail closed；在真实 Provider、MCP、恢复和 Windows canary 完成前，DSH 不属于已验收宿主。
...
Provider/Host 错误也在同一边界归一化。明确包含额度耗尽、429/限流、上下文窗口、token/output limit 或模型容量不足的错误（包括 `ModelAccountTpmRateLimitExceeded`、`insufficient_quota`、`context_length_exceeded` 等机器可读拼写），不论来自 Codex 事件、Codex 流异常、OMP RPC 还是第三方 Host，都会成为同一个 `quota` 宿主错误；`AccessDenied.Unpurchased`、模型未购买/未开通/未授权则成为 `provider_authorization`，不会继续无效重连。错误消息中的文档 URL 会在匹配前移除，URL fragment（例如错误码说明页的 `#token-limit`）不能把 TPS/TPM quota 误判成上下文容量。`Reconnecting... n/m` 是宿主内部的有界重连进度；Runner 保留事件并等待该 turn 恢复或给出最终错误，但永久模型授权拒绝会立即进入可恢复阻断。明确余额耗尽仍归为 `provider_rate_limited` 并保存可恢复阻断；可识别的瞬时 429/TPS/TPM 限流按服务端提示等待一次，在不重放已记录交互的前提下换一代线程执行 resume，仍失败才阻断。每个 epoch 自动最多承载 8 条 case，避免不准确的容量元数据把长套件压成一个超长回合。只有明确的上下文/单次输出超限才允许额外调度恢复：执行阶段尚无交付、Control MCP 已验证、权威 Ledger 为空且没有交互回执时，多 case epoch 二分后用新线程继续；已有业务写入记录时保留原 epoch，只换一代线程执行 resume 协议；finalization 同样最多恢复一次。case 结果已落盘后的 checkpoint 只是可选工作记忆，超限时直接跳过并进入下一 epoch。`at capacity`、`overloaded` 等暂时供应商容量仍显示为 `provider_capacity`，但不会靠拆分或重复线程调用碰运气。恢复次数按 epoch/stage 去重，因此不会无限重启。合法 epoch 交付和已完成 case 仍按确定性合同复用。

具体映射以 Provider 官方契约为准。例如[阿里云 Model Studio 错误码说明](https://www.alibabacloud.com/help/en/model-studio/error-code)将 `429 Throttling.AllocationQuota / Allocated quota exceeded` 定义为 TPS/TPM 触发的限流，因此该文本必须映射为 `provider_rate_limited`，不能因同一消息中的文档锚点转成上下文容量。
