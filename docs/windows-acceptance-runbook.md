...
> DSH Host 尚未进入本清单的通过矩阵。只有官方 SDK runtime、真实 Provider/MCP、ACL/PwSh sandbox、写入补偿和恢复 canary 全部完成后，才能新增 DSH Windows 验收声明。
...
探索成功后应自动出现 `agent-workspace\replay\replay-manifest.json`。Environment Profile 上限为 `read` 时，验收每个 passed case 都必须确认状态为 `verified`、verification exitCode 为 0，并且生成 spec 来自最后一个完整 case attempt；该 attempt 必须从稳定 URL 导航开始并重新执行完整业务动作，不得把瞬态结果 URL 或错误页当作源用例后置条件。Core 必须确定性拒绝缺少导航、导航前已有业务动作、任何失败的 Playwright 调用或仍含脱敏/秘密占位符的轨迹。该门禁不受 intake 的单 case 风险推断影响。允许 `write`/`destructive` 的 Profile 只自动回放被判定为 `read` 的 case；`candidate` 只表示脚本已生成，不代表已回放通过。回放使用 Run 私有的 cookies/localStorage 与 sessionStorage 捕获；普通受保护业务用例必须在最终 episode 前完成认证捕获，episode/spec 只记录注入认证态后的业务路径，不重复登录。工作区临时文件应已被 Core 删除，只保留 `.agent-private` 原件。生成 config 的测试超时为 180 秒、导航超时为 90 秒；缺少任一认证态捕获或独立回放失败时不能宣称快速回归可用。写入或破坏性 case 不自动重复执行，需准备隔离回归数据后单独验证。动态验证码登录用例需另有可重复的认证 setup，不能把一次性验证码写进 spec。
...
启动后应立即看到带时间和语义标签的阶段进度；执行期间会显示当前执行宿主、批次、执行线程代次，以及当前批次的用例范围、当前用例序号/总数、来源 Excel 行、标题摘要、导航、点击/填写/选择控件、测试辅助脚本、Control MCP、工作区更新、失败和恢复等安全动作回执。进入批次后，在单条用例边界建立前会显示“正在为第 X-Y/N 条用例探索/准备，尚未进入单条用例的可验证执行边界”；收到真实 `case_execution_begin` 后才显示具体当前用例。每个动作带序号和开始、完成或失败状态；完成和失败会带耗时，重复的同一回执不会刷屏。页面 snapshot/find/evaluate 等高频观察、推理和普通 Agent 回执不会逐条显示，只会计入心跳。窗口每约 20 秒输出一次“框架仍在运行”心跳：存在活动动作时显示“当前动作”和该动作持续时间；没有活动动作时显示当前阶段、当前用例范围或当前用例、关键动作数、页面观察数、Agent 回合数，以及用例已完成/通过/产品不符预期/阻断统计。用例标题仅显示脱敏摘要，不显示用例 ID、表单值、密码、验证码、Cookie、工具参数或模型推理正文；完整脱敏事件在 `$Run\codex-agent.events.jsonl` 中供审计。如果窗口给出明确的 `blocked` 或失败结果，应按结果中的原因处理；不要仅因某个页面步骤耗时较长就关闭窗口。
...
明确的上下文/输出容量错误（例如 `context_length_exceeded`）会归为 `provider_capacity`；额度、429/限流错误（例如 `ModelAccountTpmRateLimitExceeded`、`insufficient_quota` 或 `Allocated quota exceeded`）会归为 `provider_rate_limited`。错误说明 URL 会先从匹配文本中移除，因此文档链接的 `#token-limit` 不能把 TPS/TPM quota 误判成上下文容量。`Reconnecting... n/m` 属于 AgentHost 有界重连进度；无论括号中的临时原因是什么，都必须允许它完成当前序列：恢复时同一 turn 继续，流终止或最终失败时才按真实原因处理。可识别的瞬时 429/TPS/TPM 限流会按服务端提示等待一次，换一代线程并用 resume 继续；明确余额耗尽或第二次限流仍失败时才需要恢复额度或切换 Profile 后对原 `$Run` 执行 `--resume`。每个 epoch 自动最多包含 8 条 case；只有上下文/输出超限且没有 Ledger、没有交互回执的多 case epoch 才允许二分，已有写入记录或 finalization 最多换一代线程，case 已落盘后的 checkpoint 超限直接跳过，禁止无限重试和重复业务写入。模型暂时 `at capacity`/`overloaded` 虽显示为 `provider_capacity`，但不会靠拆分或重复线程调用碰运气。验收时确认事件日志不再停在第一次 `Reconnecting... 1/5` 后由 Runner 主动关闭；若仍最终 blocked，应核对最后的 Provider 终态，而不是把单元测试通过当成长套件通过。
