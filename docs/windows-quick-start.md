...
启动窗口会持续显示带时间和语义标签的执行进度，包括 `AgentHost`、epoch 编号、thread generation，以及受控动作的安全回执。导航、点击/填写/选择控件、Control MCP、辅助 shell、工作区更新、失败和恢复会显示标准动作、序号、开始/完成/失败状态和耗时；页面 snapshot/find/evaluate 等高频观察动作、推理和普通 Agent 回执不会逐条打印，只会计入心跳。同一个工具回执重复到达时只保留一次，同一线程和短时间内完全相同的摘要也会抑制。模型或页面动作暂时没有新事件时，窗口每约 20 秒输出一次“框架仍在运行”心跳：存在活动动作时显示“当前动作”和持续时间，否则显示当前阶段、关键动作数、页面观察数、Agent 回合数和恢复状态。因此可以区分正常执行、页面观察、等待、自动重连、thread 轮换和明确阻断。进度只显示动作类别和安全摘要，不显示模型推理正文、命令内容、表单值、工具参数、Cookie、验证码或 API 信息；完整脱敏事件仍保存在 `codex-agent.events.jsonl` 供审计。
