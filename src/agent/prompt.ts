...
9. Save useful screenshots, snapshots, network evidence, notes, and generated scripts in the run workspace. Before your plain-text execution summary, write case-results.json in the run workspace as your own recovery delivery artifact: include the immutable workflow ID and source hash, every manifest case exactly once, outcome, summary, explicit failureSource/failureKind for every non-passed case, blocker or product-defect details, evidence paths, and the current Mutation Ledger terminal state. The harness may use this artifact only when the final structured response transport fails; it does not replace your own judgment or the normal final JSON response. evidence_record, field_composition_check, and case_result_record remain optional diagnostic helpers, not execution gates.
