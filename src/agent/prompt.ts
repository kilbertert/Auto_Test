...
8. Before delivering a passed case, call browser_storage_state({ filename: 'replay-storage-state.json' }) and browser_evaluate({ function: '() => ({ origin: location.origin, entries: Object.fromEntries(Object.entries(sessionStorage)) })', filename: 'replay-session-storage.json' }) outside the case episode. For an ordinary protected business case, capture this state before its final episode.
...
4a. For an image verification code, use the image from the current browser session, OCR it, then fill and submit the visible form with snapshot refs or stable selectors. Reuse an installed OCR tool (check ddddocr first) instead of searching the filesystem or installing packages. If it expires, refresh and retry at most twice. Do not reverse-engineer application bundles, guess private login APIs, or bypass the visible authentication flow.
...
11. Before producing a passed result for any case, save the current reusable browser authentication state outside every case episode after authentication has been established. For an ordinary protected business case, capture it before the final episode so that episode records only the authenticated business path. Call browser_storage_state({ filename: 'replay-storage-state.json' }) for cookies/localStorage, then browser_evaluate({ function: '() => ({ origin: location.origin, entries: Object.fromEntries(Object.entries(sessionStorage)) })', filename: 'replay-session-storage.json' }) for sessionStorage. These are replay setup, not case actions. Core validates both captures, moves them into private storage, removes the temporary files, and injects them into a fresh BrowserContext for deterministic verification. Do not cite these temporary filenames as final evidence paths; cite retained screenshots, snapshots, or evidence records instead. Do not use browser_sessionstorage_list for this capture because it exposes values in the tool response.
