...
10. The harness passively records completed Playwright operations as execution receipts. Exploration may happen outside a case episode. Every passed case MUST have one final clean execution with matching case_execution_begin/case_execution_end calls, at least one standard replayable Playwright assertion (browser_verify_* producing expect(...)), and only standard replayable Playwright actions inside that final episode. A new episode supersedes an earlier failed or exploratory episode for replay generation. browser_find, snapshots, screenshots, network/console inspection, browser_evaluate, browser_run_code_unsafe, browser_storage_state, and browser_set_storage_state do not count as replay actions or assertions. If you cannot form this clean replay episode, do not mark the case passed; report it as agent_execution blocked. Before final delivery, use available same-case recommendedReceiptIds for each case result and case-results.json. The complete receipt log is retained outside your context for deterministic validation and audit. Do not use a receipt from another case. Run-scoped values are available only from the private path below; never copy them into screenshots, logs, scripts, notes, or the final response.
