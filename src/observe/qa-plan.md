...
**frp 公网隧道切片(2026-09-03 执行)**:端到端公网验证通过——`easy dashboard --port 11880 --token <令牌>`(回环绑定+固定端口,端口已登记 DEVELOPMENT-PORT-REGISTRY.md 11880)+ frpc 隧道(auto-test-dashboard-frp.service → https://auto-test.ranlei.work)。六项验证:无凭据 401(frp Basic Auth 层);仅 Basic Auth 无面板令牌 **401(本次修复的缝隙:显式令牌在回环绑定下同样强制校验,因为回环端口可经隧道被外部触达)**;仅面板令牌无 Basic Auth 401;双重凭据页面 200 渲染/API 正常;HTTPS 公网侧终结(证书验证零错误),面板令牌不经明文传输。tests/observe-remote-access.test.ts 新增两测试:固定端口绑定/释放/复用、回环+显式令牌全路由强制校验。

**Run journal 存储切片(#198,2026-09-22 执行,Linux x64 / Node 24.x)**:观测面对运行日志的读取改经 `RunArtifactStore`(聚合摘要的 Mutation Ledger 行不再自推 `.agent-private` 路径)。tests/observe-run-journal.test.ts 7 个测试自动化覆盖(含决策表:pending / compensated / 空 journal / journal 缺失 / journal 引用 Manifest 之外 case)——失败 Run 的业务残留行与 store 回读条目一致(parity);响应正文零 `OBS-JOURNAL-SECRET-MARKER`、零 `.agent-private`、零 `mutation-ledger` 字样(出口只计数不转发私有内容);ledger 引用清单 Manifest 之外的 case 或文件缺失时回落到"无法从当前结果确认",不把别的 Run 的条目计成业务残留。只读硬边界未被削弱:仍只绑定回环/需令牌、仍只有 GET、证据 root 仍钉死 `agent-workspace/evidence`,journal 路径推导全仓只剩 `run-artifact-store.ts` 一处(tests/run-artifact-store.test.ts 布局归属守卫)。
