...
describe('Codex agent workspace', () => {
  ...
  it('stages raw run inputs and enables the complete Playwright capability set without copying provider secrets', async () => {
    ...
    // The run-root key the Control MCP server derives the journal layout from is
    // the only journal path persisted; no stale artifact path can redirect it.
    expect(controlConfig.mutationLedgerPath).toBe(workspace.mutationLedgerPath)
    expect(controlConfig.caseResultsPath).toBeUndefined()
    expect(controlConfig.environmentRequirementsPath).toBeUndefined()
    expect(controlConfig.executionReceiptsPath).toBeUndefined()
    expect(controlConfig.fieldCompositionPath).toBeUndefined()
